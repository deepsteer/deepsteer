...
(``examples/generate_persona_corpus.py``).  The persona-voice corpus is
*known-engaging* — Step 1 (`outputs/phase_d/c10_inference_patch/`) already
...
Usage::
    ...
    python examples/step2_persona_steering.py

    # Just one condition (e.g., gate check)
    python examples/step2_persona_steering.py --conditions none
...
DEFAULT_PROBE = Path("outputs/phase_d/c10_v2/persona_probe.json")
...
def main() -> None:
    ...
    if not args.corpus.exists():
        raise SystemExit(
            ...
            "`python examples/generate_persona_corpus.py` first."
        )
