#!/usr/bin/env python3
"""Shared direction-extraction utilities used across the paper program.

Originally written for Paper 5's Phase 2 (comprehension-compliance) study and
now the common kernel imported by Papers 5, 6, 7, and Direction 1 as
...
def effective_dimensionality(
    ...
    *,
    ...
    """PCs explaining >= threshold of variance in a set of direction vectors.

    ``centered=True`` (default, the Paper 5/6 convention) subtracts the mean
    before the SVD, measuring spread *around the centroid*. ``centered=False``
    keeps the raw vectors, measuring dimensionality *about the origin* — the
    uncentered eff-rank Direction 1 uses for its V_moral subspace (a shared
    non-zero mean is real subspace structure there, not a nuisance to remove).
    The distinction is load-bearing, so it is an explicit argument rather than
    two silently-diverging copies.
    """
    mat = np.stack(direction_list).astype(np.float64)
    if centered:
...
def unit_vector(v: np.ndarray) -> np.ndarray:
    """Return ``v`` scaled to unit L2 norm (the shared ``_unit`` helper)."""
