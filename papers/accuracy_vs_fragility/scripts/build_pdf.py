...
Runs `make pdf` in `papers/accuracy_vs_fragility/build/` (which invokes
pandoc on each `sections/*.md`, then latexmk on `main.tex`), then copies
the resulting `build/main.pdf` to `papers/accuracy_vs_fragility.pdf` at
the repo root.

Usage (from project root):
    python papers/accuracy_vs_fragility/scripts/build_pdf.py
    python papers/accuracy_vs_fragility/scripts/build_pdf.py --clean
    python papers/accuracy_vs_fragility/scripts/build_pdf.py --output path/to/out.pdf
"""
...
DEFAULT_OUTPUT = PAPER_DIR.with_suffix(".pdf")  # papers/accuracy_vs_fragility.pdf
