...
**Path convention.** Paths in this document and in `sections/*.md` are *relative to the paper directory* (`papers/accuracy_vs_fragility/`). So `outputs/phase_c4_compositional/...` resolves to `papers/accuracy_vs_fragility/outputs/phase_c4_compositional/...` from the project root, and `scripts/phase_c4_compositional.py` resolves to `papers/accuracy_vs_fragility/scripts/phase_c4_compositional.py`. Project-root-relative paths are used in CLI invocations (e.g. `python papers/accuracy_vs_fragility/scripts/phase_c4_compositional.py`) and in `RESEARCH_PLAN.md` / `RESEARCH_BRIEF.md` / `README.md`. Library code (`deepsteer/...`) lives at the project root and is unaffected by the paper-directory layout.
...
Three controls — leave-lexeme-out splits, paraphrase transfer, adversarial lexical swap — were originally scoped here to bring the standard moral probe to parity with the persona probe (`papers/moe_output_dilution/outputs/phase_d/c8/`). The compositional probe (§3.2) is a strictly stronger version of two of those three by construction (leave-lexeme-out and adversarial-swap), so the operative control scope is now an open decision. **See "Validity controls" under Open items below for the three scope alternatives (Original Appendix C / Match C8 battery / Hybrid), the per-control time breakdown, and the recommended Scope C path.** Once a scope is locked, this subsection updates with the chosen control list and ~1 paragraph of methodology; Appendix C populates with the resulting numbers.
...
| C15 probe accuracy: all conditions peak | 100.0 % @ layer 9 | `papers/moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 max |Δ acc| across conditions | 0.021 ≤ 0.03 threshold | `papers/moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 base robustness peak | layer 7, critical noise 10.0 | `papers/moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 insecure robustness peak (relocated) | layers 9–10, critical noise 10.0 | `papers/moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 insecure layers 6–7 collapse | critical noise 3.0/10.0 → 1.0 | `papers/moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 mean log-fragility diff (base vs. insecure) | 0.336 > 0.20 threshold | `papers/moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
...
For each section, work in `papers/accuracy_vs_fragility/sections/NN_section_name.md`. Stitch into `papers/accuracy_vs_fragility/main.md` (or `papers/accuracy_vs_fragility/main.tex` if going LaTeX) after individual sections are stable. Keep figures in `papers/accuracy_vs_fragility/figures/`.
