...
Runs `make pdf` in `papers/2_moe_output_dilution/build/` (which invokes
pandoc on each `sections/*.md`, then latexmk on `main.tex`), then copies
the resulting `build/main.pdf` to `papers/2_moe_output_dilution.pdf` at
the repo root.

Usage (from project root):
    python papers/2_moe_output_dilution/scripts/build_pdf.py
    python papers/2_moe_output_dilution/scripts/build_pdf.py --clean
    python papers/2_moe_output_dilution/scripts/build_pdf.py --output path/to/out.pdf
"""
...
DEFAULT_OUTPUT = PAPER_DIR.with_suffix(".pdf")  # papers/2_moe_output_dilution.pdf
