...
Runs `make pdf` in `papers/moe_output_dilution/build/` (which invokes
pandoc on each `sections/*.md`, then latexmk on `main.tex`), then copies
the resulting `build/main.pdf` to `papers/moe_output_dilution.pdf` at
the repo root.

Usage (from project root):
    python papers/moe_output_dilution/scripts/build_pdf.py
    python papers/moe_output_dilution/scripts/build_pdf.py --clean
    python papers/moe_output_dilution/scripts/build_pdf.py --output path/to/out.pdf
"""
...
PAPER_DIR = Path("papers/moe_output_dilution")
BUILD_DIR = PAPER_DIR / "build"
BUILD_PDF = BUILD_DIR / "main.pdf"
DEFAULT_OUTPUT = PAPER_DIR.with_suffix(".pdf")  # papers/moe_output_dilution.pdf
