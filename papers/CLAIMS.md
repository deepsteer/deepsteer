...
| NI-2 | **Llama decision-channel PR** | 13.5 (D3 C1 decision token) / 10.2 (D2 in-format ladder, four-family list) | two positions/harnesses; declare which is "the Llama bottleneck PR" of record, or print both with their position labels | FL beat 3 four-architecture PR bar; MN bottleneck figure |
| NI-3 | **decision-channel "position-valid" vs "position-invalid"** | GPT-OSS PR 12.79 called "position-valid (25 ceiling)" in D3 RESULTS; PAPER_PLAN §1 calls OLMo/Qwen/Llama bottlenecks "all position-invalid"; prereg §5 rule = "PR<30 → position-invalid" | write the one reconciling sentence: the bottleneck is position-**invalid for content projection-fraction** tests (band-below-null) but position-**valid for decision-direction** reads (R3 cosine, GPT-OSS refusal projection). The "25 ceiling" is a separate MoE PR sanity gate, not the content rule. | FL beat 3–4; MN A2 section |
...
| D3-16 | Llama's disengage is coherent at layers 8/12/14 (−0.12/−0.11/−0.20, CIs exclude 0) but incoherent at layer 16 (−0.014) → EARLY-COMMITMENT (crystallizes before the read layer); OLMo's disengage is coherent at 16 (−0.62), so OLMo commits at/after the read layer. | CI / VERIFIED | FL | none stated | none |
...
| P1-04 | The raw late-vs-early layer-depth fragility gradient (7–15×) is **largely activation scale**: under RMS normalization it collapses to ~2× and the ordering fails at 8/37 checkpoints. | **ERRATA-CLASS CONFIRMED → v2 (F1 resolved).** v1 (arXiv:2606.11375, 9 Jun 2026) lacks §4.4; states the raw gradient as **abstract Finding 2** + Table 2 (late 10.0 / early 1.8) + Figs 3–4, with only a passing §5.2 scale hedge (no quantified control). ESCALATE erratum wording (abstract-level). | DUO | §4.4; `figure_4_rms_control.pdf` | figure_4_rms_control |
| P1-05 | The raw post-saturation fragility decline (σ* 18.3→4.7) disappears under RMS normalization (flat 13.8→15.0) — a scale artifact, not encoding change. | **ERRATA-CLASS CONFIRMED → v2 (F1 resolved).** v1 states the decline raw (Fig 3, "10→6"); §5.2 hedge exists but no correction. | DUO | §4.4 | — |
