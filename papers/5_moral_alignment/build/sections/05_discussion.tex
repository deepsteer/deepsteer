...
\textbf{Alignment is a wiring problem.} The four findings form a single argument.
...
\textbf{Why aligned behavior is removable.} This gives a mechanistic account of a
...
\textbf{Weak coupling is the vulnerability, and a lever.} The near-zero coupling
...
compliance \emph{depend} on the moral subspace, so that the refusal mechanism and the
...
\textbf{The empty row.} No OLMo-3 state, from the first pre-training checkpoint to the
...
\textbf{Limitations.} The coupling and behavioral measurements use 48 scenarios and
