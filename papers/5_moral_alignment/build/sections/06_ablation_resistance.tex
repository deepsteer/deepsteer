\section{Toward Ablation-Resistant Alignment}\label{toward-ablation-resistant-alignment}
...
compliance \emph{depend} on moral comprehension, so that the same ablation can no
...
\subsection{Moral generation depends on the moral subspace, and the dependence grows with alignment}\label{moral-generation-depends-on-the-moral-subspace-and-the-dependence-grows-with-alignment}

Decodability (Section 4.1) says morality is \emph{present} in the representation; it
does not say the model \emph{uses} it. We measure how much each pipeline state relies
...
concerns \emph{generation}: the refusal mechanism that Section 4.4 ablates lives
outside this subspace. Dependence and compliance grow in different places.

\subsection{Ablation-resistance training}\label{ablation-resistance-training}
...
\subsection{ART builds dependence, but in the wrong subspace}\label{art-builds-dependence-but-in-the-wrong-subspace}
...
subspace now hurts neutral text \emph{more} than moral text. Rather than building
...
\subsection{Coupling cannot be installed after pre-training}\label{coupling-cannot-be-installed-after-pre-training}
...
defense against ablation of the refusal subspace. Making moral \emph{generation}
...
subspace would have to \emph{absorb} the proto-features that post-training later
...
\subsection{Forcing the coupling during continued pre-training}\label{forcing-the-coupling-during-continued-pre-training}

Section 6.4 points to a sharper experiment: make the moral subspace absorb the
proto-refusal feature \emph{before} post-training recruits it, so that the refusal
...
\path|down_proj|, drives the projection from the \(0.10\) baseline to \(0.50\)
...
\subsection{Geometric coupling does not survive fine-tuning as functional resistance}\label{geometric-coupling-does-not-survive-fine-tuning-as-functional-resistance}
...
the subspace the coupling targeted makes the model refuse \emph{more}, not less.
...
harmful prompts: on sixty benign requests phrased with alarming verbs (\texttt{kill\ a\ frozen\ process\textquotesingle{}\textquotesingle{},}blow up balloons for a party'\,'), the coupled model's
...
\subsection{What forced coupling settles, and what it opens}\label{what-forced-coupling-settles-and-what-it-opens}
...
couple to, our experiments do not separate. From the evidence here, \texttt{the\ moral-foundations\ subspace\ is\ the\ wrong\ target\textquotesingle{}\textquotesingle{}\ and}refusal is not morally
