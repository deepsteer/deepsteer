\section{Steering Dose--Response Tables}
\label{app:dose-response}

Tables~\ref{tab:dose-layer4}--\ref{tab:dose-layer12} report specificity (on-target $\Delta$ minus off-target $\Delta$) for all six foundations at each injection amplitude and layer.

\begin{table}[h]
\centering
\small
\begin{tabular}{l rrrrr}
\toprule
Foundation & $\alpha{=}1$ & $\alpha{=}2$ & $\alpha{=}5$ & $\alpha{=}10$ & $\alpha{=}20$ \\
\midrule
Care      & $-0.08$ & $-0.29$ & $+0.12$ & $+2.55$ & $+3.50$ \\
Fairness  & $-0.06$ & $+0.02$ & $+0.22$ & $+1.28$ & $+1.43$ \\
Liberty   & $+0.17$ & $+0.11$ & $+1.10$ & $+3.31$ & $+2.29$ \\
Loyalty   & $+0.35$ & $+0.83$ & $+1.01$ & $+2.85$ & $+4.68$ \\
Authority & $+0.19$ & $+0.57$ & $+1.32$ & $+2.95$ & $+4.30$ \\
Sanctity  & $+0.26$ & $+0.68$ & $+1.96$ & $+5.04$ & $+6.47$ \\
\midrule
\textbf{Mean} & $+0.14$ & $+0.32$ & $+0.95$ & $+3.00$ & $+3.78$ \\
\bottomrule
\end{tabular}
\caption{Steering injection specificity at layer 4.}
\label{tab:dose-layer4}
\end{table}

\begin{table}[h]
\centering
\small
\begin{tabular}{l rrrrr}
\toprule
Foundation & $\alpha{=}1$ & $\alpha{=}2$ & $\alpha{=}5$ & $\alpha{=}10$ & $\alpha{=}20$ \\
\midrule
Care      & $-0.00$ & $+0.00$ & $+0.51$ & $+2.40$ & $+4.41$ \\
Fairness  & $-0.05$ & $+0.09$ & $+0.20$ & $+0.31$ & $+0.60$ \\
Liberty   & $+0.17$ & $+0.24$ & $+0.97$ & $+2.32$ & $+2.54$ \\
Loyalty   & $+0.27$ & $+0.55$ & $+1.03$ & $+2.19$ & $+3.73$ \\
Authority & $+0.04$ & $+0.21$ & $+0.82$ & $+2.50$ & $+3.76$ \\
Sanctity  & $+0.27$ & $+0.52$ & $+1.74$ & $+4.31$ & $+5.57$ \\
\midrule
\textbf{Mean} & $+0.12$ & $+0.27$ & $+0.88$ & $+2.34$ & $+3.44$ \\
\bottomrule
\end{tabular}
\caption{Steering injection specificity at layer 8.}
\label{tab:dose-layer8}
\end{table}

\begin{table}[h]
\centering
\small
\begin{tabular}{l rrrrr}
\toprule
Foundation & $\alpha{=}1$ & $\alpha{=}2$ & $\alpha{=}5$ & $\alpha{=}10$ & $\alpha{=}20$ \\
\midrule
Care      & $+0.16$ & $+0.33$ & $+0.75$ & $+1.54$ & $+3.36$ \\
Fairness  & $+0.03$ & $+0.05$ & $+0.04$ & $+0.14$ & $+0.86$ \\
Liberty   & $+0.02$ & $-0.05$ & $-0.11$ & $-0.07$ & $+0.44$ \\
Loyalty   & $+0.06$ & $+0.14$ & $+0.41$ & $+0.79$ & $+1.66$ \\
Authority & $+0.04$ & $+0.10$ & $+0.27$ & $+0.73$ & $+1.89$ \\
Sanctity  & $+0.20$ & $+0.37$ & $+0.61$ & $+1.17$ & $+2.16$ \\
\midrule
\textbf{Mean} & $+0.08$ & $+0.15$ & $+0.33$ & $+0.72$ & $+1.73$ \\
\bottomrule
\end{tabular}
\caption{Steering injection specificity at layer 12. Layer 12 shows weaker injection effects than earlier layers, complementing its stronger ablation effects.}
\label{tab:dose-layer12}
\end{table}
