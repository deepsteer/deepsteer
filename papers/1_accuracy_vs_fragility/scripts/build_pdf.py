#!/usr/bin/env python3
"""Build the Paper 1 PDF and copy it to the canonical location.

Runs `make pdf` in `papers/1_accuracy_vs_fragility/build/` (which invokes
pandoc on each `sections/*.md`, then latexmk on `main.tex`), then copies
the resulting `build/main.pdf` to `papers/1_accuracy_vs_fragility.pdf` at
the repo root.

Usage (from project root):
    python papers/1_accuracy_vs_fragility/scripts/build_pdf.py
    python papers/1_accuracy_vs_fragility/scripts/build_pdf.py --clean
    python papers/1_accuracy_vs_fragility/scripts/build_pdf.py --output path/to/out.pdf
"""
...
DEFAULT_OUTPUT = PAPER_DIR.with_suffix(".pdf")  # papers/1_accuracy_vs_fragility.pdf
