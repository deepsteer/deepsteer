...
**Path convention.** Paths in this document and in `sections/*.md` are *relative to the paper directory* (`papers/1_accuracy_vs_fragility/`). So `outputs/phase_c4_compositional/...` resolves to `papers/1_accuracy_vs_fragility/outputs/phase_c4_compositional/...` from the project root, and `scripts/phase_c4_compositional.py` resolves to `papers/1_accuracy_vs_fragility/scripts/phase_c4_compositional.py`. Project-root-relative paths are used in CLI invocations (e.g. `python papers/1_accuracy_vs_fragility/scripts/phase_c4_compositional.py`) and in `RESEARCH_PLAN.md` / `RESEARCH_BRIEF.md` / `README.md`. Library code (`deepsteer/...`) lives at the project root and is unaffected by the paper-directory layout.
...
## Abstract (draft v4 — story-driven rewrite, current)
...
**v4 vs. v3.** v4 is a presentation rewrite, not a finding change. v3 led with numbers (4K of 36K, 95 %, 1K / 2K / 5K / 6K, 10.0 → 1.7, 0.10 / 5.11 / 2.46 / 0.12, ≈80 %) and asked the reader to reconstruct the story from them; v4 leads with the methodological problem (probing accuracy saturates → most of pre-training is invisible), introduces fragility as the complementary move, and headlines each finding with a one-sentence insight. Numbers move to §4 where they belong. Underlying findings, decision-rule outcomes, and number sources below are unchanged.

**Underlying-result rationale (carried from v3, unchanged in v4).** Phase C4 result: compositional probe onset step 5K under 4-seed averaging (4-seed mean acc 0.709 ± 0.025, plateau 0.769 ± 0.030; per-seed onsets 4K / 4K / 7K / 7K), between sentiment (2K) and syntax (6K). This is Outcome B from the original placeholder set: the lexical-accessibility framing is partially right; single-token moralized vocabulary is decoded earlier than compositional moral integration. The other two outcome variants (A: synchronous onset; C: failure-to-decode) were ruled out by the data and are removed from this plan. The 3-seed compositional fragility replication landed `decline_real` (gap 2.19 vs. max std 0.84; both decision-rule conditions pass), which is now incorporated into Finding 2's abstract sentence rather than left as "reproduced independently." The compositional onset value updated from 4K (seed-42 only) to 5K (4-seed mean) when Figure 1 was regenerated with the 4-seed band; per-seed onsets span 4K-7K and the 4-seed mean of mean accuracy first crosses 0.70 at step 5K. Numbers sources: `outputs/phase_c4_compositional/RESULTS.md`, `outputs/phase_c4_compositional/3seed/decision.json`, and `outputs/phase_c4_compositional/3seed/aggregate_per_checkpoint.json`.
...
Inject Gaussian noise of magnitude σ into cached test-set activations after probe training; record the σ at which probe accuracy drops below the fragility threshold (0.6 by default, i.e. chance + 0.1 on a binary task). Logarithmic noise sweep [0.1, 0.3, 1.0, 3.0, 10.0]. The minimum σ at which accuracy falls below threshold is the layer's *critical noise*. Per-layer critical noise across all transformer layers gives the *fragility profile*; mean across layers gives `mean_critical_noise` as a scalar summary. The same `MoralFragilityTest` infrastructure runs against the standard moral probe (Phase C1) and the compositional probe (Phase C4); methodology generality is established by reuse, not by reimplementation.
...
Three controls — leave-lexeme-out splits, paraphrase transfer, adversarial lexical swap — were originally scoped here to bring the standard moral probe to parity with the persona probe (`papers/2_moe_output_dilution/outputs/phase_d/c8/`). The compositional probe (§3.2) is a strictly stronger version of two of those three by construction (leave-lexeme-out and adversarial-swap), so the operative control scope is now an open decision. **See "Validity controls" under Open items below for the three scope alternatives (Original Appendix C / Match C8 battery / Hybrid), the per-control time breakdown, and the recommended Scope C path.** Once a scope is locked, this subsection updates with the chosen control list and ~1 paragraph of methodology; Appendix C populates with the resulting numbers.
...
The figure that does the most work for the methodological thesis: two-panel comparison, shared training-step x-axis. Top panel: mean probing accuracy over training, a sharp sigmoid 0 → ~95 % between steps 0 and 4K, then completely flat for the remaining ~33K steps. Bottom panel: fragility, with an initial jump alongside accuracy, then continued downward drift in early layers through step 36K, with late layers holding at maximum robustness. Visually self-evident: top panel is "ceiling reached, no information"; bottom panel is "still moving."
...
Numbers source: `outputs/phase_c1/RESULTS.md` (1B trajectory, 37 checkpoints, dense). 7B fragility evolution from `outputs/phase_b/` is supporting evidence, showing the same pattern at the 7B headline scale, less dense checkpoint sampling.
...
#### 5.3 Generalization beyond pre-training: fine-tuning fingerprints

The fragility-detects-what-accuracy-misses pattern reproduces under a different stimulus. Applying the same moral-probe + fragility battery to LoRA adapters from narrow insecure-code fine-tuning of a fully-trained OLMo-2 1B (C15 reframed experiment; numbers source: `papers/2_moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md`) shows identical probing accuracy across base / insecure / secure conditions (max |Δ| = 0.021 ≤ 0.03, all three conditions at 100% peak accuracy layer 9) but a fragility-locus shift of 2–3 layers under insecure-code specifically: the base-model robustness peak at layer 7 (critical noise = 10.0) *relocates* to layers 9–10 under insecure-code LoRA, while layers 6–7 collapse from critical noise 10.0/3.0 to 1.0. Secure-code LoRA tracks base much more closely. Mean log-fragility difference = 0.336 grid units (above 0.20 differential threshold).

This is the cleanest possible demonstration of fragility's value: same model, same probe, same dataset, same accuracy everywhere, with structure visible *only* on the fragility metric. The methodology generalizes from pre-training dynamics to fine-tuning fingerprints. Paper 2 (in preparation) extends this line to MoE architectures, asking whether fine-tuning affects specific experts rather than specific layers.

**Promoted from one-paragraph reference to full subsection.** This finding was originally Paper 2 §4.4; with Paper 2 pivoting to MoE architectures, it now serves Paper 1 directly as evidence that fragility generalizes beyond the pre-training trajectory setting.
...
Restate the thesis: probing accuracy is the wrong instrument for studying training dynamics; fragility is right. Position fragility as a methodology contribution that the alignment-during-pre-training research program can adopt for any probing-based question, not just moral representations. Restate the lexical→compositional gradient (1K / 5K) as the science finding that earns the methodology its keep. A single-token framing of "moral encoding emerges at step 1K" overstates what the probes recover, while "lexically-marked moralized vocabulary at 1K, compositional moral integration at 5K, syntactic competence at 6K" is the honest reading. End on two open questions Phase C4 leaves: (a) does the compositional plateau at ~0.77 lift with model scale (7B / 32B), and (b) does the fragility-resolves-what-accuracy-misses pattern that we establish for the standard moral probe and conditionally for the compositional probe extend to other probing-based investigations of pre-training? Both gates Phase E and broader adoption of the methodology.

### Acknowledgments / AI disclosure (between §6 and references)

Itemized disclosure of Claude usage (Claude Code on Opus 4.6 and 4.7) in three categories: (1) code scaffolding (analysis scripts, LaTeX build pipeline, NeurIPS integration); (2) prose drafting (initial section drafts, four abstract iterations, title brainstorming); (3) bibliography assembly (initial entries from training-data recall, flagged for arXiv-page cross-check). Followed by an author-responsibility paragraph stating that experimental design, dataset construction, scientific claims, and final wording remain the author's responsibility. Rendered via the `\begin{ack}…\end{ack}` environment from `neurips_2025.sty`, which produces an "Acknowledgments and Disclosure of Funding" header.
...
- **C. Standard moral probe validity controls (parity with persona probe).** Final control list depends on which scope is chosen under "Validity controls" in Open items (recommended Scope C: paraphrase transfer + C8 battery). The compositional probe (§3.2) addresses the strongest version of "your probe is just reading vocabulary" by construction; these controls are for parity with Paper 2's reported controls rather than load-bearing for the C4 gradient finding.
...
5. **Figure 5: Fine-tuning fingerprint (C15 fragility-locus shift).** Three-condition layer-wise fragility profile (base / insecure-code LoRA / secure-code LoRA). Probing accuracy identical across conditions; fragility profile shows the robustness peak relocating from layer 7 to layers 9–10 under insecure-code. Source: `papers/2_moe_output_dilution/outputs/phase_d/c15_reframed/c15_fragility.png`; restyle for paper consistency. *Pairs with Figure 4 as a second controlled comparison where accuracy is flat and fragility moves.*
...
| C15 probe accuracy: all conditions peak | 100.0 % @ layer 9 | `papers/2_moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 max |Δ acc| across conditions | 0.021 ≤ 0.03 threshold | `papers/2_moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 base robustness peak | layer 7, critical noise 10.0 | `papers/2_moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 insecure robustness peak (relocated) | layers 9–10, critical noise 10.0 | `papers/2_moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 insecure layers 6–7 collapse | critical noise 3.0/10.0 → 1.0 | `papers/2_moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| C15 mean log-fragility diff (base vs. insecure) | 0.336 > 0.20 threshold | `papers/2_moe_output_dilution/outputs/phase_d/c15_reframed/RESULTS.md` |
| Compositional moral onset step (4-seed mean) | 5K | `outputs/phase_c4_compositional/3seed/aggregate_per_checkpoint.json` |
| Compositional moral onset mean acc (4-seed) | 0.709 ± 0.025 | `outputs/phase_c4_compositional/3seed/aggregate_per_checkpoint.json` |
| Compositional moral plateau mean acc (4-seed, step 36K) | 0.769 ± 0.030 | `outputs/phase_c4_compositional/3seed/aggregate_per_checkpoint.json` |
| Compositional moral per-seed onset range | 4K-7K (seeds: 42=4K, 43=4K, 44=7K, 45=7K) | `outputs/phase_c4_compositional/3seed/aggregate_per_checkpoint.json` |
...
- **Lexical→compositional gradient, not lexical-accessibility hedge.** Phase C4 turned the original "we frame this as lexical accessibility" hedge into a quantitative four-point gradient (1K / 2K / 5K / 6K under 4-seed averaging; per-seed compositional onset range 4K-7K). Every sentence that could be read as "moral reasoning emerges" gets rephrased as "moralized lexical distinctions become decodable at step 1K; compositional moral integration at step 5K." The compositional probe is now a real §3 subsection (§3.2) and a real §4 finding (§4.1, §4.2), not an appendix hedge.
...
- **C15 reframed promoted to full §5.3 subsection with figure.** Originally one-paragraph cross-reference to Paper 2; now a proper subsection with the three-condition fragility profile (base / insecure / secure). Paper 2 has pivoted to MoE architectures, so this finding belongs here as the strongest single demonstration that fragility generalizes beyond pre-training dynamics. Add the fragility profile figure (from `papers/2_moe_output_dilution/outputs/phase_d/c15_reframed/c15_fragility.png`) as Figure 5 or a §5.3 companion panel.
- **Foundation emergence and causal-probing divergence go to appendix.** Not load-bearing for the methodological thesis.
- **Compositional probe partially obviates standard moral validity controls.** The original plan required leave-lexeme-out / paraphrase / adversarial swap controls for the standard moral probe before submission. The compositional probe (§3.2) is a strictly stronger version of leave-lexeme-out and adversarial swap by construction; only paraphrase transfer is genuinely new evidence. Active scope decision is the *Validity controls* item under Open items: recommended **Scope C** (paraphrase transfer + Paper-2 C8 battery) trades 3 of the 6 originally-scoped controls for tighter Paper-2 parity at lower total time. Decision held until prose drafting is otherwise complete.
...
- **Validity controls (deprioritized — open scope decision).** Leave-lexeme-out, paraphrase, adversarial swap controls for the standard moral probe. The compositional probe (§3.2) is a strictly stronger version of two of these three (#1 and #3 below) by construction; only paraphrase transfer (#2) is genuinely new evidence. The persona probe in companion work (C8) ran a *different* control battery (TF-IDF baseline + content-clean→leaky transfer + OOD transfer), not the three the original Appendix C outline lists; for true parity with Paper 2 the right move is to replicate the C8 battery on the moral probe. Three open scope alternatives, not yet decided:

    | Scope | What it includes | Total time | Subsumption claim strength |
    |-------|------------------|-----------:|----------------------------|
    | **A. Original Appendix C** | Leave-lexeme-out + paraphrase transfer + adversarial swap on standard moral probe | 4-6 hours | Full coverage; #1 and #3 partly redundant with §3.2 |
    | **B. Match C8 battery** | TF-IDF baseline + content-clean→leaky split + OOD transfer on standard moral probe | 3-4 hours | Direct parity with Paper 2's reported controls |
    | **C. Hybrid** | Paraphrase transfer (the new evidence) + C8 battery (parity) | ~4 hours | Cleanest |

  Per-control breakdown for reference:

  - *Leave-lexeme-out* (~1 hour): for each of ~15-20 high-frequency moral lexemes (`murder`, `kill`, `steal`, …), retrain `LayerWiseMoralProbe` with all pairs containing that lexeme held out; evaluate on the held-out pairs. Compute is trivial (probes are seconds; one OLMo-2 1B final activation cache, ~1 min). Real cost is code (~30 min, mostly the adapter for held-out filtering) + tabulation (~30 min). Subsumed by §3.2 compositional probe.
  - *Paraphrase transfer* (~2-3 hours): generate 2-3 paraphrases per test-set pair (~150 new pairs), evaluate a probe trained on originals against the paraphrased versions. LLM path (Claude API, ~30 min + API key) is faster; deterministic-template path is more reproducible but slower to author. ~5 min activation collection on new texts. **This is the only control the compositional probe does not subsume**, since it tests robustness to surface-form variation, not single-token lexical reading.
  - *Adversarial lexical swap* (~1-2 hours): construct ~50 pairs where surface features (sentence length, lexeme position, function-word distribution) are decoupled from the moral label. Trivial compute; real cost is careful dataset authoring. Subsumed by §3.2 compositional probe.
  - *C8 battery for parity* (~3-4 hours combined):
    - TF-IDF baseline on the standard moral probe (~30 min): a few minutes of code, 10s of compute. Likely high (that's the whole reason the compositional probe exists); the number itself becomes part of the §4.1 framing.
    - Content-clean→leaky split for standard moral pairs (~1 hour): needs designing the categorization (which MFT foundations or pair-construction patterns count as content-clean vs. content-leaky). Substantive design work.
    - OOD transfer to a small held-out moral fixture (~1-2 hours construction): build ~30-40 OOD pairs from a different distribution (morally-loaded news headlines, template-different moral content); activation collection + probe eval are quick.

  Plus appendix writing (~1 hour): populate Appendix C with the resulting tables, per-control discussion, and the explicit subsumption claim where it applies.

  **Recommended path: Scope C.** Paraphrase transfer + C8 battery hits both the "novel control" and "parity with Paper 2" goals with the lowest total time. Leave-lexeme-out and adversarial swap are dropped with explicit subsumption-by-compositional-probe footnotes in Appendix C. Reserve the option to add either back if a reviewer specifically asks. **Decision deferred until prose drafting is otherwise complete.** Once a scope is locked, update §3.6 and Appendix C in lockstep with the resulting numbers and discussion.
...
- **Citation verification pass (before submission).** The 10 references added in commit 6f80138 (`hewitt2019control`, `pimentel2020information`, `voita2020mdl`, `hu2022lora`, `zou2023repe`, `hubinger2024sleeper`, `biderman2023pythia`, `olsson2022induction`, `nanda2023progress`, `olmo2_2025`) were written from training-data recall, so author lists, venues, page numbers, and arXiv IDs need to be cross-checked against the actual papers. Two specifically warrant attention: (1) `olmo2_2025` uses `{OLMo Team}` as the author, which renders as "OLMo Team 2025", but the actual paper may have specific lead authors (likely Walsh, Soldaini, Bhagia, Groeneveld, et al.) and the cite form should match; (2) `hubinger2024sleeper` truncates the author list with `others` after eight names and should be verified against the published author order. Cheapest version is a single agent pass that fetches each arXiv abstract page and diffs the bib entry against it. ~30 minutes of work; trivial to schedule.
- **AI disclosure finalization (before submission).** The acknowledgments / AI-disclosure block landed in `build/main.tex` between §6 and references (rendered via `\begin{ack}` from `neurips_2025.sty`). Items to confirm at submission time: (a) the exact Claude model name(s) used during the drafting and analysis windows (currently stated as "Opus 4.6 and 4.7"; tighten to dated model IDs if the venue requires; NeurIPS does not but some workshops do); (b) any additional categories of AI usage beyond code / prose / bibliography that should be itemized (e.g., review / critique passes, dataset-construction assistance for the compositional moral pairs, currently stated only as "training-data recall" for bib entries). Verify against the venue's specific AI-disclosure policy once submission target is chosen.
...
For each section, work in `papers/1_accuracy_vs_fragility/sections/NN_section_name.md`. Stitch into `papers/1_accuracy_vs_fragility/main.md` (or `papers/1_accuracy_vs_fragility/main.tex` if going LaTeX) after individual sections are stable. Keep figures in `papers/1_accuracy_vs_fragility/figures/`.
