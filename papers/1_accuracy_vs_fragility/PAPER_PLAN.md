...
- **Validity controls (deprioritized — open scope decision).** Leave-lexeme-out, paraphrase, adversarial swap controls for the standard moral probe. The compositional probe (§3.2) is a strictly stronger version of two of these three (#1 and #3 below) by construction; only paraphrase transfer (#2) is genuinely new evidence. The persona probe in companion work (C8) ran a *different* control battery (TF-IDF baseline + content-clean→leaky transfer + OOD transfer), not the three the original Appendix C outline lists; for true parity with Paper 2 the right move is to replicate the C8 battery on the moral probe. Three open scope alternatives, not yet decided:
    ...
    |-------|------------------|-----------:|----------------------------|
    | **A. Original Appendix C** | Leave-lexeme-out + paraphrase transfer + adversarial swap on standard moral probe | 4-6 hours | Comprehensive; #1 and #3 partly redundant with §3.2 |
...
- **Citation verification pass (before submission).** The 10 references added in commit 6f80138 (`hewitt2019control`, `pimentel2020information`, `voita2020mdl`, `hu2022lora`, `zou2023repe`, `hubinger2024sleeper`, `biderman2023pythia`, `olsson2022induction`, `nanda2023progress`, `olmo2_2025`) were written from training-data recall — author lists, venues, page numbers, and arXiv IDs need to be cross-checked against the actual papers. Two specifically warrant attention: (1) `olmo2_2025` uses `{OLMo Team}` as the author, which renders as "OLMo Team 2025" — the actual paper may have specific lead authors (likely Walsh, Soldaini, Bhagia, Groeneveld, et al.) and the cite form should match; (2) `hubinger2024sleeper` truncates the author list with `others` after eight names and should be verified against the published author order. Cheapest version is a single agent pass that fetches each arXiv abstract page and diffs the bib entry against it. ~30 minutes of work; trivial to schedule.
