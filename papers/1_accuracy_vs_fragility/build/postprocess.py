...
CITES_PARENS = [
    ...
    (r"\(Groeneveld\s+et\s+al\.,\s+2024;\s+Team\s+OLMo,\s+2025a;\s+Team\s+OLMo,\s+2025b\)",
     r"\citep{groeneveld2024olmo,olmo2_2025,olmo3_2025}"),
    # Single (Author, Year)
    ...
    (r"\(Team\s+OLMo,\s+2025a\)", r"\citep{olmo2_2025}"),
    (r"\(Team\s+OLMo,\s+2025b\)", r"\citep{olmo3_2025}"),
...
FIGURE_INSERTS: list[tuple[str, str]] = [
    ...
    (
        ...
        (
            ...
            "plateaus, from raw $\\sigma^*\\!\\approx\\!18.3$ at step~4K to "
            "$\\approx\\!4.7$ at step~36K (the values tabulated in Table~2). "
            "Note: this bottom-panel trajectory is \\emph{raw} $\\sigma^*$ and "
            "is largely activation scale (Sec.~4.4); the scale-free "
            "(RMS-normalized) version is flat (13.8~to~15.0).}\n"
