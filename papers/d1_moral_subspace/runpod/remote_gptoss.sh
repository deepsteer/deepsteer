...
#     DISK_GB=220 REMOTE_SCRIPT=papers/d1_moral_subspace/runpod/remote_gptoss.sh ./run_session.sh
set -uo pipefail
...
D="$REPO_DIR/papers/d1_moral_subspace/scripts"
...
VALIDATE="$VALIDATE" MATCH_LAYER=16 python "$D/phase2_extract.py" --mft \
  --model "$INSTRUCT_MODEL" --out "$REPO_DIR/papers/d1_moral_subspace/outputs/phase2/instruct"
...
VALIDATE="$VALIDATE" MATCH_LAYER=12 python "$D/phase2_extract.py" \
  --model "$GPTOSS_MODEL" --out "$REPO_DIR/papers/d1_moral_subspace/outputs/phase2/gpt_oss"
VALIDATE="$VALIDATE" MATCH_LAYER=12 python "$D/phase2_axis_extract.py" \
  --model "$GPTOSS_MODEL" --out "$REPO_DIR/papers/d1_moral_subspace/outputs/phase2/gpt_oss_axis"
