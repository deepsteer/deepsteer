...
# Launch via:
#   REMOTE_SCRIPT=papers/d1_moral_subspace/runpod/remote_think_g3.sh ./run_session.sh
...
VALIDATE="$VALIDATE" python "$D/phase2_extract.py" --model "$THINK_MODEL" \
  --out "$REPO_DIR/papers/d1_moral_subspace/outputs/phase2/think"
echo ">> [2/3] fables + ethics axis on Think ..."
VALIDATE="$VALIDATE" python "$D/phase2_axis_extract.py" --model "$THINK_MODEL" \
  --out "$REPO_DIR/papers/d1_moral_subspace/outputs/phase2/think_axis"
