...
# Launch via:
#   REMOTE_SCRIPT=papers/d1_moral_subspace/runpod/remote_think_refusal_only.sh ./run_session.sh
...
VALIDATE="$VALIDATE" PILOT_N="$PILOT_N" MAX_NEW_TOKENS="$MAX_NEW_TOKENS" P23_N="$P23_N" \
  ...
  python "$REPO_DIR/papers/d1_moral_subspace/scripts/phase3_think_refusal.py" \
  --model "$THINK_MODEL"
