...
keystone (`deepsteer.reasoning.token_positions`):
...
# The post-instruction token count and the `</think>` boundary both come from the
# shared core reasoning utilities (originally Paper 7). The boundary helper is a
# string-match in the decoded text + binary-search back to the token index --
# immune to the BPE merges that broke a fixed-token-subsequence matcher. Reused,
# not reimplemented.
from deepsteer.reasoning import think_io as ti
