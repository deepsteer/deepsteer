...
def errbar(ax, y, x, lo, hi, color, label=None, xmax=1.0):
    ...
    if label:
        ax.text(min(hi, xmax * 0.55) + 0.015, y - 0.28, label, va="center", ha="left", fontsize=9, color="#52514e")


def headline(fig, text):
    fig.text(0.01, 1.02, text, fontsize=10, ha="left", va="bottom", color=INK)
...
def fig_ladder():
    ...
    for ax, rows, key, color, title, xl in (
        ...
        ax.set_xlim(-0.02, 1.0); ax.set_ylim(len(rows) - 0.4, -0.7); ax.set_xlabel(xl); ax.set_title(title, fontsize=10, loc="left")
...
def fig_strictness():
    ...
    for ax, src, key, color, title in ((axes[0], b, "measurement_minus_null_paired", BLUE, "Binary: excess over null (paired)"),
        ...
        ax.set_yticks(range(3)); ax.set_yticklabels(["L0: original frame", "L1: + four-frame majority", "L2: + all frames agree"], fontsize=9.5)
        ax.axvline(0, color=INK, lw=0.8); ax.set_xlim(-0.08, 0.42); ax.set_ylim(2.6, -0.7); ax.set_title(title, fontsize=10, loc="left")
...
def fig_families():
    ...
    for ax, src, key, color, title in ((axes[0], b["per_family"], "rate", BLUE, "Binary KDG by family"),
        ...
        ax.set_yticks(range(len(fams))); ax.set_yticklabels([names[f] for f in fams], fontsize=9.5)
        ax.set_xlim(-0.02, 0.75); ax.set_ylim(len(fams) - 0.4, -0.7); ax.axvline(0, color="#c3c2b7", lw=0.8); ax.set_title(title, fontsize=10, loc="left")
...
def fig_three_cell():
    ...
    for i, (name, t) in enumerate(src.items()):
        ...
        ax.text(i - 0.18, w["base_gap_rate"] + 0.006, f"{w['base_gap_rate']:.3f}", ha="center", fontsize=9, color=INK)
        ax.text(i + 0.18, w["instruct_gap_rate"] + 0.006, f"{w['instruct_gap_rate']:.3f}", ha="center", fontsize=9, color=INK)
        ax.text(i, -0.028, f"n shared = {w['n']}", ha="center", fontsize=9, color="#52514e")
        out.append((name, w["n"], w["base_gap_rate"], w["instruct_gap_rate"]))
    ax.set_xticks(list(xs)); ax.set_xticklabels(list(src), fontsize=9.5); ax.set_ylim(0, 0.2)
    ax.set_ylabel("raw-frame gap rate"); ax.legend(frameon=False, fontsize=8, loc="upper right")
    ax.set_title("Raw-frame gap, base vs instruct on shared scenarios (mass floor 0.5)", fontsize=10, loc="left")
...
def fig_f4():
    ...
    for ax, src, key, color, title in ((axes[0], {p: b[p]["per_family"]["F4"] for p in ("anthropic", "openai")}, "rate", BLUE, "F4 binary KDG by writer"),
        ...
        ax.set_yticks([0, 1]); ax.set_yticklabels(["Claude-written", "GPT-written"], fontsize=9.5)
        ax.set_xlim(-0.02, 0.95); ax.set_ylim(1.6, -0.7); ax.axvline(0, color="#c3c2b7", lw=0.8); ax.set_title(title, fontsize=10, loc="left")
    ...
    fig.text(0.01, -0.02, f"Blind human read of the same 24 scenarios: the construction label agreed on {agree['claude']}/{n['claude']} Claude-written and {agree['gpt']}/{n['gpt']} GPT-written items.", fontsize=9, color="#52514e", ha="left")
