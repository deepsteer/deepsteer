...
\textbf{GPT-OSS-20B} \citep{openai2025gptoss} is the primary: a mixture-of-experts model
whose reasoning was learned by reinforcement learning under deliberative alignment,
making its chain of thought a trained behavior. \textbf{DeepSeek-R1-Distill-Llama-8B} and
\textbf{DeepSeek-R1-Distill-Qwen-14B} \citep{deepseek2025r1} acquired their reasoning by
...
\subsection{Extracting harm comprehension and refusal at separate token positions}\label{extracting-harm-comprehension-and-refusal-at-separate-token-positions}
...
\texttt{\textless{}\textbar{}Assistant\textbar{}\textgreater{}\textless{}think\textgreater{}} opener and the GPT-OSS harmony assistant opener alike,
...
\subsection{Subspaces and prompts}\label{subspaces-and-prompts}
...
\path|harmful_behaviors| / \path|harmless_alpaca| prompts), with a held-out
...
\subsection{Reply-inversion and the readout problem}\label{reply-inversion-and-the-readout-problem}
...
answer position (a reasoning-close followed by an \texttt{Answer:} prefix) and comparing
...
\subsection{Decodable is not causal, and held-out gating}\label{decodable-is-not-causal-and-held-out-gating}
