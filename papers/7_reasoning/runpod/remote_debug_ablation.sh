#!/usr/bin/env bash
# One-shot ablation diagnostic on GPT-OSS (isolates direction vs machinery vs harness).
set -uo pipefail
...
echo ">> debug ablation"
python papers/7_reasoning/scripts/debug_ablation.py --key gpt_oss_20b \
  --prompts papers/5_moral_alignment/refusal_prompts.json --n 2 --max-new-tokens "${MAXTOK:-512}"
echo ">> debug exit $?"
