#!/usr/bin/env bash
# Remote Phase 2a runner — position-resolved trace capture + length disentanglement.
# Executed ON the pod by run_session.sh (REMOTE_SCRIPT=.../remote_phase2a.sh).
set -uo pipefail
...
# GPT-OSS needs the torch 2.6 trio (matched torchvision/torchaudio); only when in the run.
case ",$MODELS," in
...
echo ">> Phase 2a [VALIDATE=$VALIDATE] $ARGS"

VALIDATE="$VALIDATE" python papers/7_reasoning/scripts/run_phase2a.py $ARGS
RC=$?
echo ">> run_phase2a exit code: $RC"
exit $RC
