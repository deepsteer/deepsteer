...
\subsection{Refusal is residual and orthogonal to morality in every family}\label{refusal-is-residual-and-orthogonal-to-morality-in-every-family}
...
\caption{Refusal-direction decomposition at the depth-\(0.5\) headline layer. No
...
\label{tab:decomp}}\tabularnewline
\toprule\noalign{}
...
\emph{most} moral projection (Qwen) is also the one with the \emph{most} diffuse refusal
...
\subsection{Ablating refusal preserves the linear moral representation everywhere}\label{ablating-refusal-preserves-the-linear-moral-representation-everywhere}
...
At each model's best ablation layer, ablating refusal leaves the \emph{linear moral
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{Ablation and the comprehension battery at each model's swept ablation
...
not fully strip.
\label{tab:battery}}\tabularnewline
\toprule\noalign{}
...
\subsection{The behavioral dissociation is clean in OLMo-3 and Qwen2.5}\label{the-behavioral-dissociation-is-clean-in-olmo-3-and-qwen2.5}

In the two families where the single-direction ablation fully removes refusal, it
also leaves \emph{behavioral} moral judgment intact. OLMo's persona-shift compliance
...
\subsection{Llama-3.1: refusal entangled with moral judgment}\label{llama-3.1-refusal-entangled-with-moral-judgment}
...
\textbf{The drop is refusal-direction-specific.} Perturbing the same weight matrices
with random Gaussian noise of \emph{identical per-matrix Frobenius norm} to the refusal
...
\textbf{The drop is dose-dependent.} \Cref{fig:dose} sweeps the fraction \(\alpha\) of
