...
\textbf{A representational property generalizes; a behavioral one does not.} The
cleanest way to state the result is to separate two things that ``comprehension''
can mean. The \emph{linear moral representation}, what a probe can decode from
...
\emph{behavioral moral judgment}, what the model actually outputs when asked to judge a
...
\textbf{Refusal removability is not universal.} A side result of the ablation step is
...
\textbf{The Llama coupling, stated precisely.} What we have shown about Llama is that
...
this an \emph{entanglement} between refusal and behavioral moral judgment that is
\emph{dose-dependent} and \emph{refusal-direction-specific}. We deliberately do not say that
...
\textbf{Why this matters for where to look next.} In this one model the two
...
\textbf{Limits.} The co-occurrence of ablation-resistance and moral entanglement rests
...
\textbf{Safety.} Every measurement here is diagnostic. We characterize an existing
...
refusal harder to remove. The reverse question, whether refusal \emph{should} be more
