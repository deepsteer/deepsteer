...
\textbf{Models.} All six checkpoints are public Hugging Face releases: OLMo-3 7B
(\path|allenai/Olmo-3-1025-7B|, \path|allenai/Olmo-3-7B-Instruct|) \citep{olmo3_2025},
Qwen2.5-7B (\path|Qwen/Qwen2.5-7B|, \path|Qwen/Qwen2.5-7B-Instruct|) \citep{qwen2025qwen25},
and Llama-3.1-8B (\path|meta-llama/Llama-3.1-8B|, \path|meta-llama/Llama-3.1-8B-Instruct|)
\citep{grattafiori2024llama3}. Layer counts are 32, 28, and 32; hidden sizes 4096,
3584, and 4096. Models load in \texttt{float16}.

\textbf{Conventions.} Layer indices are set by depth fraction: the stable band is the
...
\textbf{Contrast sets.} The refusal direction uses the Arditi et al.~harmful/harmless
...
\textbf{Pipeline.} Phase 1 (decomposition) extracts, per family, the base moral and
...
\(W \gets W - \alpha\, r\, r^{\top} W\) over attention \path|o_proj| and MLP \path|down_proj|
at all layers, with \(\alpha=1\) unless swept.

\textbf{Statistics.} Moral-judgment drops are paired per-scenario bootstrap estimates
...
\textbf{Determinism and compute.} Probe initialization and dataset resampling are
