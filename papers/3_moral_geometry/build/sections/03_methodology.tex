...
\tightlist
\item
  Train a linear probe (fixed seed) on clean activations.
\item
...
\item
  ...
  noise seeds.
\item
  The \textbf{critical noise} \(\sigma^*\) is the smallest \(\sigma\) where the
...
\citep{reblitzrichardson2026dilution} is foundation-uniform. As \Cref{differential-fragility-across-frameworks}
...
cross-architecture difference, which an RMS-normalized control (\S4.11)
