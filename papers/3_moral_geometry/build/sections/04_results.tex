...
using three alternative methods and compare.
...
layer 0). Bootstrap stability is high: mean pairwise cosine between
100 bootstrap resamples is 0.957, indicating that the direction
estimates are robust to sampling variability.

\textbf{LEACE directions.} LEACE \citep{belrose2023leace} extracts
directions via Fisher's linear discriminant:
\(\mathbf{d}_f = \boldsymbol{\Sigma}^{-1}(\overline{\mathbf{a}}_{\text{moral}} -
\overline{\mathbf{a}}_{\text{neutral}})\), where
\(\boldsymbol{\Sigma}\) is the pooled within-class covariance.
This whitening decorrelates the foundation directions, producing
near-orthogonal vectors (mean pairwise cosine 0.07--0.11 across
layers, vs.~0.27--0.45 for probe weights). The key geometric
finding replicates: effective dimensionality is 5 at all layers.
However, the MFT dendrogram match fails at all 16 layers ---
the whitening removes the shared moral-salience component that
encodes inter-foundation correlations, eliminating the
individualizing/binding clustering. LEACE thus recovers the
same 5D moral subspace but rotates the basis to maximize class
separation, confirming that the dimensionality finding is
method-invariant while the clustering structure is a property
of the raw activation geometry. Bootstrap stability is moderate
(0.678), and alignment with mean-diff and probe-weight directions
is 0.30 and 0.14 respectively, reflecting the distinct
...
bootstrap stability is low (0.558), and the resulting directions
show weak alignment with probe-weight directions
(\(|\cos| = 0.07\)--\(0.29\)).

The root cause is the centering step. Standard RepE centers the
paired differences before PCA, subtracting the mean difference
vector. But the mean difference \emph{is} the moral-vs-neutral
signal --- it is precisely the mean-diff direction. After centering,
the residuals have near-uniform variance across \({\sim}200\)
dimensions (PC1 explains only 3.9\% of the centered variance,
and the singular value spectrum is nearly flat). In
\(\mathbb{R}^{2048}\) with \({\sim}32\) samples, PC1 of these
diffuse residuals fits noise. Confirming this diagnosis, uncentered
PCA recovers the mean-diff direction almost exactly
(\(\cos = 0.995\)). This negative result shows that RepE's centering
step, while appropriate for concepts with structured within-pair
variance, destroys the signal when paired differences are dominated
by a single direction with diffuse residuals.
...
\textbf{Replication on a larger dataset.} To confirm that the
geometric findings are not artifacts of the 240-pair probing
dataset, we replicated the analysis on a 1{,}200-pair dataset
(200 pairs per foundation across three text registers: declarative,
situational, and narrative). Mean-difference directions on the
larger dataset reproduce the core findings: effective dimensionality
is 5 at layers 0--14 (dropping to 4 at layer 15), and the MFT
dendrogram match holds at 12 of 16 layers. The integration
signature is stable across a 5\(\times\) increase in dataset size.
